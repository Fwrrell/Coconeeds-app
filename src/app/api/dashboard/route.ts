...
export async function GET(request: Request) {
  ...
  try {
    ...
    // --- 1. KPI Aggregation ---
    const whereKopdes: any =
      kopdesId && kopdesId !== "ALL" ? { petani: { kopdesId } } : {};
    ...
    const [
      ...
      prisma.wtbListing.count({
        where: { status: "DEAL", updatedAt: { lt: thirtyDaysAgo } },
      }),
    ]);
    ...
    for (const batch of rawBatches) {
      ...
      if (batch.type.toUpperCase() === "KOPRA")
        existing.kopra += batch.totalWeight;
      else if (batch.type.toUpperCase() === "SABUT")
        existing.sabut += batch.totalWeight;
    ...
    const activities = [
      ...recentBatches.map((b) => ({
        ...
        type: "qc" as const,
        title: `QC ${b.type}`,
        description: `${b.totalWeight} kg — ${b.status}`,
      })),
      ...recentPengiriman.map((p) => ({
        id: p.id,
        date: p.createdAt,
        type: "logistics" as const,
        title: `Kapal ${p.namaKapal}`,
        description: `${p.rute} — ${p.status}`,
      })),
      ...recentDeals.map((d) => ({
        id: d.id,
        date: d.updatedAt,
        type: "deal" as const,
        title: `Deal ${d.komoditas}`,
        description: `${(d.targetWeight / 1000).toFixed(1)} Ton @ Rp${d.dealPrice?.toLocaleString("id-ID")}`,
      })),
    ];
    ...
    return NextResponse.json({
      kpi: {
        totalPetani: {
          value: totalPetani,
          growth: calculateGrowth(totalPetani, prevTotalPetani),
        },
        kargoGudang: { value: kargoGudang._sum?.totalWeight ?? 0, growth: 0 },
        kargoBerlayar: {
          value: kargoBerlayar._sum?.totalWeight ?? 0,
          growth: 0,
        },
        totalDeals: { value: totalDeals, growth: totalDeals - prevTotalDeals },
      },
